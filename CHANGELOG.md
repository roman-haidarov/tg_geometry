...
- FeatureSource bulk execution now runs file read, JSON traversal, and TG geometry parsing in a C-only no-GVL phase before Ruby materialization / Index ownership transfer. When a Fiber scheduler is active, that heavy phase runs through an internal worker thread and the current fiber is parked via scheduler block/unblock so other fibers can continue running.
